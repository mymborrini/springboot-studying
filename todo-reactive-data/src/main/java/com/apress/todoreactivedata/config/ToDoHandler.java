...
public class ToDoHandler {
  ...
  public Mono<ServerResponse> newToDo(ServerRequest request) {
    ...
    return ServerResponse.ok().contentType(APPLICATION_JSON).body(fromPublisher(toDo.flatMap(this::save), ToDo.class));
  }
  ...
  private Mono<ServerResponse> findById(String id) {
    ...
    return toDo
        .flatMap(t -> ServerResponse.ok().contentType(APPLICATION_JSON).body(fromObject(t)).switchIfEmpty(notFound));
